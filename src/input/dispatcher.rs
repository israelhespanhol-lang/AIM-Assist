...
impl EventDispatcher {
    pub fn new(tx: mpsc::Sender<Event>, settings: DispatcherSettings) -> Option<Self> {
        let interception = match ic::Interception::new() {
            Some(interception) => interception,
            None => {
                error!("Could not create Interception context. Make sure Interception driver is installed!");
                return None;
            }
        };
        ...
        info!("Toggle Key: {:?}", settings.toggle_key);
        info!("Excluded Keys: {:?}", settings.excluded_keys);
    ...
    pub fn run(&mut self) {
        ...
        info!(
            "Interception loop running. Press Toggle Key to activate/deactivate gamepad emulation."
        );

        loop {
            let device = self.interception.wait();

            let num_strokes = self.interception.receive(device, &mut strokes) as usize;

            for i in 0..num_strokes {
                let pass_through = self.process_stroke(device, strokes[i]);
                if pass_through {
    ...
    fn process_stroke(&mut self, device: ic::Device, stroke: ic::Stroke) -> bool {
        match stroke {
            ...
            ic::Stroke::Mouse {
                ...
                if x != 0 || y != 0 {
                    if let Err(e) = self.tx.send(Event::MouseMove(x, y)) {
    ...
    fn toggle_active(&mut self) {
        ...
        if self.active {
            info!(">>> GAMEPAD EMULATION: ACTIVE <<<");
            // Release any currently pressed keys/buttons to the OS to avoid stuck inputs
            self.release_held_inputs();
        } else {
            info!(">>> GAMEPAD EMULATION: DISABLED (Pass-through) <<<");
    ...
    fn process_key(&mut self, device: ic::Device, code: ic::ScanCode, state: KeyState) -> bool {
        ...
        if self.active {
            if changed_state {
                let _ = self.tx.send(Event::Keyboard(code, state));
            }
            false
        } else {
            true
    ...
    fn process_mouse_state(&mut self, device: ic::Device, state: ic::MouseState) {
        ...
        for &(flag_down, flag_up, button) in &table {
            ...
            let key_state = if state.contains(flag_down) {
                KeyState::Down
            } else {
                KeyState::Up
            };
            ...
            if self.active {
                let _ = self.tx.send(Event::MouseButton(button, key_state));
            }
        }
    }
}
